...
async function openFloatWindow() {
  try {
    const taskType = document.getElementById('taskType')?.value || appData.types[0] || '其他';
    const content = document.getElementById('content')?.value?.trim() || '';
    await invoke('save_float_task', { task: { taskType, content } });
    await invoke('open_float_window');
  } catch (e) {
    console.error('open_float_window failed', e);
    alert('打开悬浮窗失败：' + e);
  }
}
...
async function closeFloatWindow() {
  try {
    ...
    console.error('close_float_window failed', e);
  }
}

async function setFloatProperties(alwaysOnTop) {
  try {
    await invoke('set_float_window_properties', { alwaysOnTop });
  } catch (e) {
    console.error('set_float_window_properties failed', e);
  }
}
